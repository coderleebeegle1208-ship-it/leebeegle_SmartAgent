...
- **주간·월간 비용**: `오늘 한 일` 창 위의 `하루 / 주간 / 월간` 버튼으로 이번 주(월~일)·이번 달 합계와 날짜별 비용 막대를 봅니다. 홈 줄에는 `이달 $금액`이 함께 표시됩니다.
- **자주 쓰는 지시**: 입력창의 `+` › `자주 쓰는 지시`에서 지시를 저장해 두고 누르면 입력창에 들어갑니다. 입력창에 적어 둔 내용은 `새로 저장`을 누르면 그대로 담깁니다. 많이 쓴 순서로 정렬됩니다.
- **자동 백업**: 서버가 켜져 있는 날마다 한 번 `data/backups/YYYY-MM-DD/`에 대화 기록(app.sqlite)과 설정(config.json)을 복사하고 최근 14일치를 보관합니다. 설정 › `자동 백업`에서 마지막 백업 시각을 보거나 `지금 백업`을 누를 수 있습니다. 다른 폴더(예: 클라우드 동기화 폴더)에 두려면 `data/config.json`에 `"backupDir": "경로"`를 적습니다. 복구는 서버를 끄고 백업 폴더의 두 파일을 `data/`에 덮어쓰면 됩니다.
