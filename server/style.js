...
- 긴 작업을 백그라운드로 띄웠으면 곧바로 mcp__approver__progress 도구에 그 로그 파일 경로(log_file)와 작업 이름(label)을 알려 대표 폰에 진행률 막대가 뜨게 한다. 로그에 "37%"나 "3/10" 같은 진행 표시가 찍히도록 실행한다. 단계가 여러 개인 작업(파일 여러 개 처리 등)은 단계가 끝날 때마다 percent를 알린다.
